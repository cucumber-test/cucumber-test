...
console.log(chalk.green('Specs:', specs));
...
if (program.cloud) {
    ...
    if (program.cloud === true) {
        if (typeof base.cloud !== 'string') {
            console.log(chalk.red(`option --cloud need to have params or config.js {base: {cloud:'cloudprovider'}} set`));
            process.exit(1);
        }
    } else {
        cloud = program.cloud;
    ...
    console.log(chalk.green(`Run from: ${cloud}`));
    ...
    if (provider === 'saucelabs') {
        ...
        options.port = 80;
    } else if (provider === 'perfecto') {
        options.host = config.perfecto.host;
        options.path = '/nexperience/perfectomobile/wd/hub';
        options.port = 80;
    }
    if (provider === 'perfecto' || options.user && options.key) {
        if (config[provider]) {
            remoteConfig = _merge(remoteConfig, config[provider]);;
        }
    } else {
        console.log(chalk.red(`option --cloud need to have env of user & key of the cloud set`));
        process.exit(1);
    }
}
...
if (remoteConfig.remote) {
    console.log(chalk.green('Remote>>>:', remoteConfig.remote));
...
console.log(chalk.green('Timeout/Retry/I:', `${timeout}/${retry}/${base.instances}`));
...
cct --android [deviceName:platformVersion]
    ...
    if (browser.match(/\:[a-zA-Z]+\d+/) && program.config === undefined) {
        console.log(chalk.red(`Browser: ${browser} need to have option --config`));
        process.exit(1);
    }
...
console.log(chalk.green('Browser:'), chalk.cyan(browser, program.uaIphone ? '--uaIphone' : '', program.uaGalaxy ? '--uaGalaxy' : '', program.android ? '--android' : ''));
...
if (program.config) {
    options.capabilities.forEach((obj, idx) => {
        ...
        if (capability) {
            ...
            console.log(chalk.magenta(JSON.stringify(capability, null, 4)));
        }
    });
}
...
console.log(chalk.green('Tags:', tagExpression));
...
try {
    ...
    wdio.run().then(code => {
        console.timeEnd("Run");
        console.log(chalk.green('Normal exit:', code));
        process.exit(code);
    }, error => {
        console.timeEnd("Run");
