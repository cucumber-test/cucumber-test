...
module.exports = function (falseCase, expectedPath) {
    ...
    if (falseCase) {
        expect(currentUrl).to.not.equal(expectedPath, 'expected path not to be "' + currentUrl + '"');
    } else {
        expect(currentUrl).to.equal(expectedPath, 'expected path to be "' + expectedPath + '" but found ' + ('"' + currentUrl + '"'));
    }
};