...
var _browserName = void 0;

function isBrowserTag(tagName) {
    var browserId = _browserName();
...
var beforeFeature = function beforeFeature(event) {
    var desiredCapabilities = global.browser.desiredCapabilities;
    var browserName = desiredCapabilities.browserName;
    var name = global.browser.options.name;

    var newScenarios = [];

    _browserName = function _browserName() {
        return browserName.toLowerCase().replace(/ /g, '');
    };
    var file = event.uri.split('/features/').pop();
